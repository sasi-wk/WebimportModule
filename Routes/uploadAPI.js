...
router.get('/errorInfo',function (req, res, next,) {
    ...
    pool.connect(async function (err, client, done) {
        ...
        console.log(Errqs+uploaded)
        const query = await client.query('SELECT uploaded, ref, err_msg FROM service_sent where err_msg is not null AND uploaded =($1)',[new Date(req.query.ref)])
        for (let i = 0; i < query.rows.length; i++) {
            results.push(query.rows[i])
